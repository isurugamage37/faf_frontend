...
//   styleUrl: './foreign-flows.component.css'
// })
// export class ForeignFlowsComponent {
      ...
      if (ctx) {
        ...
        this.chart = new Chart(ctx, {
          ...
          data: {
            ...
            datasets: [{
              label: `Total ${this.tabDisplayName} (in million tons)`, // Dataset label
              data: values, // Set dynamic values as data
              backgroundColor: 'rgb(196, 40, 32 , 0.6)', // Adjusted color
          ...
          options: {
            ...
            plugins: {
              title: {
                ...
                text: `Total ${this.tabDisplayName} Over the Last ${this.yearsCount} Years`, // Updated chart title
                font: {
  ...
  loadForeignImportTabDetails() {
    this.apiService.loadForeignImportTabDetails().subscribe({
      next: (response) => {
        ...
        console.log('Response:666666666666666677777777777777799', response.foreign_origin);
      },
      error: (error) => {
        console.error('Error:', error);
      },
  ...
  loadForeignExportTabDetails() {
    this.apiService.loadForeignExportTabDetails().subscribe({
      next: (response) => {
        ...
        this.commodity = response.commodity;
        this.years = response.state_year;
        console.log('Response:6666666666666666777777777777777iiiiiiii', response);
      },
      error: (error) => {
        console.error('Error:', error);
      },
  ...
  loadTranspotationModeDetails() {
    ...
    this.apiService.loadTranspotationModeDetails(payload).subscribe({
      next: (response) => {
        ...
        console.log('traspotationModeDetailstraspotationModeDetails:', this.traspotationModeDetails);
  ...
  postForeignExport() {
    ...
    this.apiService.postForeignExport(payload).subscribe({
      next: (response: Blob) => {
        ...
        link.href = url;
        link.download = 'foreign_exports.csv'; // Set the default download filename
  ...
  postForeignImport() {
    ...
    this.apiService.postForeignImport(payload).subscribe({
      next: (response: Blob) => {
        ...
        const url = window.URL.createObjectURL(response);
        link.href = url;
        link.download = 'foreign_imports.csv'; // Set the default download filename
        link.click(); // Trigger the download
        window.URL.revokeObjectURL(url); // Clean up the object URL
      },
      error: (error) => {
